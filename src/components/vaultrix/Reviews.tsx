...
export function Reviews() {
  ...
  const visible = [
    REVIEWS[i],
    REVIEWS[(i + 1) % REVIEWS.length],
    REVIEWS[(i + 2) % REVIEWS.length],
  ];
      ...
      <div className="container mx-auto px-4">
        ...
        >
          ...
          <p className="mt-3 text-sm text-muted-foreground">
            Based on real customer vouches from our Discord community.
          </p>
        </motion.div>
            ...
            {visible.map((r, idx) => (
              ...
              >
                ...
                <div className="mt-5 flex items-center gap-3 pt-4 border-t border-white/5">
                  <img
                    src={
                      ASSETS.reviewAvatars[r.name] ??
                      `https://placehold.co/150x150/111111/ffffff?text=${encodeURIComponent(r.name[0])}`
                    }
                    alt={r.name}
                    loading="lazy"
        ...
        <div className="mt-8 flex justify-center gap-1.5">
          {REVIEWS.map((_, idx) => (
            <button
              key={idx}
              onClick={() => setI(idx)}
              className={`h-1.5 rounded-full transition-all ${idx === i ? "w-8 gradient-orange-purple" : "w-1.5 bg-white/20"}`}
